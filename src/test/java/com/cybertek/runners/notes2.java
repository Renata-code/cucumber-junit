...
How the Hooks class is connected to the feature files?
    ...
    - Whenever we create Hooks class with @Before/@After (imported from cucumber), it will automatically check the GLUE PATH.
    - CukesRunners looks for the GLUE PATH for the HOOKS annotations.
...
---> The whole purpose of being able to have more than one BEFORE/AFTER is to be able to apply different conditions to different scenarios.
    ex: Lets say we need to connect to database for only SOME of the scenarios.
        ...
        - For this reason, we can create a separate Before where we do additional configuration specific to those few scenarios.
---> BEFORESTEP :
    -> This annotation will apply BEFORE each step
---> AFTERSTEP :
    -> This annotaion will apply AFTER each step
...
#6 -> Parameterization
    ...
    - Parameterization allows us to pass test data and run our scenarios on different data sets without having to go into code and change anything
